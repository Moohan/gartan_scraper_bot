...
def setup_temp_db():
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE crew (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE)")
...
def test_crew_duration_string_and_null():
    ...
    try:
        ...
        c.execute("INSERT INTO crew_availability (crew_id,start_time,end_time) VALUES (?,?,?)", (1, start, end))
        conn.commit()
