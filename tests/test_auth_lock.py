...
@patch("run_bot.gartan_login_and_get_session")
def test_run_bot_does_not_create_lock_on_non_credential_auth_error(
    mock_login, tmp_path
):
    mock_login.side_effect = AuthenticationError(
        "Some other reason", is_credential_failure=False
    )
    ...
    with patch("run_bot.config.auth_lock_path", str(lock_file)):
        with patch("sys.argv", ["run_bot.py", "--max-days", "1"]):
            # Non-credential AuthenticationError should not trigger the auth lock.
            # It will be caught and logged in run_bot.main, but should not sys.exit(2).
            run_bot.main()
