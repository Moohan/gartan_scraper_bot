...
def get_db():
    if has_app_context():
        ...
        return g.db
    # Fallback for standalone scripts/tests outside request context
...
def get_crew_status_batch(now: datetime) -> List[Dict]:
    """Fetch all crew and their current availability in a single batch query."""
    with get_db() as conn:
        # Use a single query with LEFT JOIN and GROUP BY to get all crew and their current status.
        # MAX(ca.end_time) ensures we get the longest active block if multiple exist.
        query = """
        ...
        for r in rows:
            c_dict = dict(r)
            end_time_raw = r["current_end_time"]
...
def get_available_crew_details(now: datetime) -> List[Dict]:
    """Fetch full details for all currently available crew members."""
    with get_db() as conn:
        query = """
            SELECT c.*
            FROM crew c
...
def root():
    try:
        now = datetime.now()
        crew_data = get_crew_status_batch(now)
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                available_crew = get_available_crew_details(now)
                return jsonify(
