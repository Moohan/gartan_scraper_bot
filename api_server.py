...
def format_availability_display(end_time: datetime, now: datetime) -> str:
    """Standard format for 'Until' display."""
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules using already-fetched crew data dictionaries."""
    if not crew_data:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Batch fetch crew metadata and current availability in one query
            # We use LEFT JOIN to include all crew even if not available
            rows = conn.execute(
                """
