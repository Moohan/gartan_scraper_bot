...
def get_db():
    """Get a database connection, reusing it if already open in this request context."""
    if "db" not in g:
...
def format_availability_display(end_time: datetime, now: datetime) -> Dict:
    """Format availability end time into a display dictionary."""
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    return format_availability_display(parse_dt(curr["end_time"]), now)
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules using already fetched crew data."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Helper for endpoints that only have available IDs. Fetches data and calls check_rules_from_data."""
    ...
    # Create temporary crew-like structures for check_rules_from_data
    mock_crew_data = []
    for r in rows:
        mock_crew_data.append(
            {"role": r["role"], "skills": r["skills"], "available": True}
        )

    return check_rules_from_data(mock_crew_data)
...
def root():
    try:
        ...
        # Optimization: Single JOIN to fetch crew and their current availability
        # We use a LEFT JOIN to ensure all crew are returned even if they have no availability record
        # Note: In case of multiple overlapping availability records, we pick the one with the latest end_time
        rows = conn.execute(
            """
        ...
        for r in rows:
            c = dict(r)
            if c["end_time"]:
                avail = format_availability_display(parse_dt(c["end_time"]), now)
            else:
        ...
        # Optimization: Use already-fetched data for rules check
