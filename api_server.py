...
def get_currently_available_crew(now: datetime) -> List[Dict]:
    """Fetch role and skills for crew members currently available."""
    with get_db() as conn:
        query = """
...
def get_crew_with_availability(now: datetime) -> List[Dict]:
    ...
    with get_db() as conn:
        # Use a LEFT JOIN to get all crew and their availability (if any) for 'now'
        query = """
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Check business rules against a list of available crew members.

    Accepts a list of dictionaries with 'role' and 'skills' keys.
    """
...
def root():
    try:
        now = datetime.now()
        # Optimized: Fetch crew and availability in a single query (resolves N+1)
