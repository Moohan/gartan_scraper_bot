...
def root():
    try:
        now = datetime.now()
        conn = get_db()
        # Optimized: Single query with JOIN to fetch all crew and their current availability.
        # Uses a subquery to ensure only one availability record per crew member.
        query = """
        ...
        # Use already fetched crew data for rule checking
