...
from utils import (ensure_london, get_now, is_auth_locked)
...
def get_db():
    """Get database connection, reusing if in Flask request context."""
    try:
        if g:
            if "db" not in g:
        ...
        pass
    # Fallback for non-request context (e.g. benchmarking)
...
def get_availability(target_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        if table == "crew_availability":
            row = conn.execute(
                "SELECT end_time FROM crew_availability WHERE crew_id = ? AND start_time <= ? AND end_time > ? ORDER BY end_time DESC LIMIT 1",
                (target_id, now, now),
            ).fetchone()
        elif table == "appliance_availability":
            row = conn.execute(
                "SELECT end_time FROM appliance_availability WHERE appliance_id = ? AND start_time <= ? AND end_time > ? ORDER BY end_time DESC LIMIT 1",
                (target_id, now, now),
            ).fetchone()
        else:
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if os.path.exists(config.db_path):
                ...
                # Use project helper for timezone consistency (creating aware UTC first)
                ls_dt = ensure_london(datetime.fromtimestamp(mtime, tz=timezone.utc))
