...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Core logic to check business rules from a list of crew member data dicts."""
    if not crew_data:
        return {
            "rules_pass": False,
            "rules": {},
...
def check_rules(available_ids: List[int]) -> Dict:
    """Legacy wrapper for check_rules_from_data fetching data by IDs."""
...
def root():
    try:
        now = datetime.now()
        conn = get_db()
        # Optimized query with LEFT JOIN to get crew and current availability in one go
        query = """
