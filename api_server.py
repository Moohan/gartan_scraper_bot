...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules against a list of crew data dictionaries."""
    if not crew_data:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        ...
        # We use a LEFT JOIN to ensure all crew members are returned even if they have no current availability.
        # SQLite's MAX(ca.end_time) is used with GROUP BY to pick the latest availability if multiple exist (unlikely with defrag).
        query = """
            SELECT
                c.*,
        ...
        for r in rows:
            c = dict(r)
            end_time_raw = c.pop("current_end_time")
