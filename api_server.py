...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    # Use static queries for whitelisted tables to prevent SQL injection
    with get_db() as conn:
...
def check_rules(available_ids: List[int]) -> Dict:
    ...
    rows = []
    with get_db() as conn:
        # Use individual parameterized queries to satisfy strict static analysis (Sourcery)
        # while avoiding dynamic SQL construction for IN clauses.
