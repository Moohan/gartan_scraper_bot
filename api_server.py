...
def get_db():
    """Get database connection from flask.g for persistence within request."""
    if "db" not in g:
...
def _format_avail_info(end_time: Optional[datetime], now: datetime) -> Dict:
    """Shared helper to format availability info from an end_time."""
    if not end_time:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    return _format_avail_info(parse_dt(curr["end_time"]) if curr else None, now)
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Optimized rules check that uses already-fetched crew data."""
...
def root():
    try:
        ...
        # Use MAX(ca.end_time) and GROUP BY c.id to handle multiple availability blocks
        query = """
        ...
        for r in rows:
            c = dict(r)
            end_time = parse_dt(c.pop("end_time")) if c.get("end_time") else None
            avail = _format_avail_info(end_time, now)
