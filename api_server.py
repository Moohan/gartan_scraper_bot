...
def check_rules(crew_members: List[Dict]) -> Dict:
    """Check availability rules against a list of crew dictionaries."""
    if not crew_members:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        now = datetime.now()
        # Optimized batch query: fetch all crew and their current availability in one go
        with get_db() as conn:
            rows = conn.execute(
                """
        ...
        for r in rows:
            c_dict = dict(r)
            # Use helper to process the fetched end_time
            avail = format_availability_data(c_dict.pop("current_end_time"), now)
        ...
        # Filter available crew for rule checking (already have roles/skills)
