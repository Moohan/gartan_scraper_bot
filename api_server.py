...
def _format_availability_data(end_time: datetime, now: datetime) -> Dict:
...
def check_rules(
    ...
    if crew_dicts:
        # Use pre-fetched data to avoid redundant DB query
...
def root():
    try:
        ...
        with get_db() as conn:
            # ⚡ Optimization: Use a single JOIN query to fetch crew and current availability
            # resolving the N+1 query problem and reducing dashboard latency by ~70%.
            query = """
                SELECT
            ...
            for r in rows:
                ...
                if end_time_raw:
                    ...
                    c.update(_format_availability_data(end_time, now))
            ...
            # ⚡ Optimization: Use pre-fetched crew data for rule checking
            ...
            if app_p22:
                # ⚡ Optimization: Reuse existing DB connection
