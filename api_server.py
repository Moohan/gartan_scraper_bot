...
def format_availability_data(now: datetime, end_time: Optional[Any]) -> Dict:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        ...
        return format_availability_data(now, curr["end_time"] if curr else None)
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Check business rules against a list of pre-fetched crew dictionaries (containing roles and skills)."""
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Batch fetch all crew and their current availability status using a single JOIN.
            # Use MAX(ca.end_time) and GROUP BY c.id to handle multiple availability records.
            query = """
                SELECT c.*, MAX(ca.end_time) as availability_end_time
                FROM crew c
            ...
            for r in rows:
                c = dict(r)
                avail = format_availability_data(now, c.pop("availability_end_time"))
                c.update(avail)
                crew_data.append(c)
                if c["available"]:
