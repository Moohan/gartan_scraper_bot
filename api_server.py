...
def get_db():
    """Get database connection, reusing it within the same request context."""
    try:
        if "db" not in g:
        ...
        # Fallback for standalone scripts or tests outside of Flask context
...
def close_db(error):
    """Ensure database connection is closed at the end of the request."""
...
def check_rules(crew_members: List[Dict]) -> Dict:
    """Evaluate business rules for a given list of available crew members."""
    if not crew_members:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        ...
        with get_db() as conn:
            ...
            crew_data = []
            for c in all_crew:
                end_time = parse_dt(c["end_time"]) if c["end_time"] else None
                avail = format_availability_data(end_time, now)
