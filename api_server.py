...
def get_db():
    """Get database connection, reusing it if within a Flask app context."""
...
def format_availability_data(row: Any, now: datetime) -> Dict:
    """Standardize conversion of raw DB row to availability dictionary."""
    # row can be a sqlite3.Row or a dict. Try accessing by name first.
    end_time = None
    try:
...
def get_currently_available_crew(conn: sqlite3.Connection, now: datetime) -> List[Dict]:
    """Helper to fetch all crew members currently available."""
    rows = conn.execute(
        """
        SELECT c.role, c.skills
        FROM crew c
...
def get_availability(
    ...
    """Check availability for a crew member or appliance."""
    ...
    return format_availability_data(curr, now)
...
def check_rules(crew_list: List[Dict]) -> Dict:
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Optimized single query for all crew and current availability
            query = """
        ...
        for r in rows:
            c_dict = dict(r)
            avail = format_availability_data(c_dict, now)
            crew_data.append({**c_dict, **avail})
        ...
        with get_db() as conn:
            app_p22_row = conn.execute(
                "SELECT id FROM appliance WHERE name = 'P22P6'"
            ).fetchone()
            if app_p22_row:
                p22p6_base = get_availability(
                    app_p22_row["id"], "appliance_availability", now, db_conn=conn
                )

        p22p6_avail = p22p6_base.get("available", False) and rules_res["rules_pass"]
...
def crew_avail(id):
    try:
        ...
        with get_db() as conn:
            ...
            res = get_availability(id, "crew_availability", now, db_conn=conn)
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if not app_row:
                return jsonify({"error": "Not found"}), 404
            base = get_availability(
                app_row["id"], "appliance_availability", now, db_conn=conn
            )
            if name == "P22P6":
                available_crew = get_currently_available_crew(conn, now)
                return jsonify(
    ...
    with get_db() as conn:
        ...
        if name == "P22P6":
            ...
            return {
                ...
                "end_time_display": base.get("end_time_display"),
            }
        return base
