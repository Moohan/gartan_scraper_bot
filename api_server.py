...
def _format_avail_info(end_time: datetime, now: datetime) -> Dict:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    return _format_avail_info(parse_dt(curr["end_time"]), now)
...
def root():
    try:
        ...
        # ⚡ Optimization: Use a LEFT JOIN to fetch all crew and their current availability in one query.
        # We use a subquery to select only the latest (max) end_time per crew_id to prevent duplicates
        # in the UI and avoid inflated counts in check_rules when overlapping blocks exist.
        query = """
            SELECT c.*, ca.max_end_time as avail_end_time
            FROM crew c
            LEFT JOIN (
                SELECT crew_id, MAX(end_time) as max_end_time
        ...
        for r in rows:
            c = dict(r)
            if c["avail_end_time"]:
                avail = _format_avail_info(parse_dt(c["avail_end_time"]), now)
            else:
                avail = {"available": False, "duration": None, "end_time_display": None}
            # Remove the raw end_time to keep the dict clean
            del c["avail_end_time"]
        ...
        # ⚡ Optimization: Pass pre-fetched crew_data to check_rules to avoid another query
        rules_res = check_rules(avail_ids, crew_data=crew_data)
