...
def format_availability_data(now: datetime, end_time: datetime) -> Dict:
    """Standardize the conversion of raw database timestamps into the standard availability dictionary."""
...
def get_crew_with_availability(now: datetime) -> List[Dict]:
    """Fetch all crew members and their current availability in a single JOIN query."""
    with get_db() as conn:
        # ⚡ Bolt: Using LEFT JOIN and MAX(ca.end_time) to resolve N+1 query problem.
        query = """
            SELECT
                c.*,
                MAX(ca.end_time) as availability_end_time
            FROM crew c
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        ...
        return format_availability_data(now, end_time)
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Evaluate business rules against pre-fetched crew data to determine appliance operational status."""
    ...
    # ⚡ Bolt: Eliminated redundant database queries by accepting pre-fetched crew dictionaries.
...
def root():
    try:
        now = datetime.now()
        # ⚡ Bolt: Using optimized JOIN helper to fetch all crew data in one request.
        crew_data = get_crew_with_availability(now)
        ...
        rules_res = check_rules(avail_crew)
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                ...
                return jsonify(
                    base["available"] and check_rules(avail_crew)["rules_pass"]
                )
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                ...
                avail_crew = [dict(r) for r in rows]
                if not (base["available"] and check_rules(avail_crew)["rules_pass"]):
                    return jsonify(None)
...
def get_appliance_available_data(name):
    ...
    with get_db() as conn:
        ...
        if name == "P22P6":
            ...
            avail_crew = [dict(r) for r in rows]
            return {
                "available": base["available"] and check_rules(avail_crew)["rules_pass"]
            }
