...
def check_rules(available_ids: List[int]) -> Dict:
    ...
    return check_rules_from_data(rows)


def check_rules_from_data(crew_data_list: List[Dict]) -> Dict:
    ...
    rules = {
        "total_crew_ok": len(crew_data_list) >= 4,
...
def root():
    try:
        ...
        # ⚡ Optimization: Fetch all crew and their current availability in a single JOIN query.
        # This replaces N+1 queries where N is the number of crew members.
        query = """
        ...
        # ⚡ Optimization: Fetch P22P6 availability in a single query.
...
def app_avail(name):
    try:
        ...
        if not app_res:
            return jsonify({"error": "Not found"}), 404
        base = get_availability(app_res["id"], "appliance_availability", now)
        if name == "P22P6":
            rows = conn.execute(
                "SELECT role, skills FROM crew_availability ca JOIN crew c ON ca.crew_id = c.id WHERE start_time <= ? AND end_time > ?",
                (now, now),
            ).fetchall()
            return jsonify(
                base["available"] and check_rules_from_data(rows)["rules_pass"]
            )
...
def app_dur(name):
    try:
        ...
        if name == "P22P6":
            ...
            if not (base["available"] and check_rules_from_data(rows)["rules_pass"]):
                return jsonify(None)
...
def get_appliance_available_data(name):
    ...
    if name == "P22P6":
        ...
        return {
            "available": base["available"] and check_rules_from_data(rows)["rules_pass"]
        }
...
def get_appliance_duration_data(name):
    ...
    if not app_res:
        return {"error": "Not found"}
    return get_availability(app_res["id"], "appliance_availability", now)
