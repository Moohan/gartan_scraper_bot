...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    # Use static query strings to satisfy security scanners (B608)
...
def check_rules(available_ids: List[int]) -> Dict:
    ...
    with get_db() as conn:
        ...
        # Use dynamic placeholders for IN clause with security suppression
        query = f"SELECT role, skills FROM crew WHERE id IN ({placeholders})"  # nosec B608
        rows = conn.execute(query, available_ids).fetchall()  # sourcery skip: sql-injection, avoid-sql-string-concatenation
