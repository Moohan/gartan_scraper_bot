...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Optimized: Fetch crew and their current availability in a single query
            rows = conn.execute(
                """
            ...
            for r in rows:
                c = dict(r)
                if c.get("end_time"):
                    avail = format_availability_data(parse_dt(c["end_time"]), now)
                else:
                    avail = {
                        "available": False,
                        "duration": None,
            ...
            # Fetch P22P6 appliance availability
