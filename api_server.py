...
def get_db():
    """Get a database connection, reusing it if it exists in the current request context."""
    if "db" not in g:
...
def format_availability_display(end_time: datetime, now: datetime) -> str:
    """Standardize the display of availability end times."""
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules against a list of crew member dictionaries."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Check business rules for a list of crew IDs by fetching their details."""
    ...
    # Mark all these as available for the helper
    crew_data = [dict(r, available=True) for r in rows]
    return check_rules_from_data(crew_data)
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Optimized single query for all crew and their current availability.
            # MAX(ca.end_time) combined with GROUP BY ensures robustness against
            # any overlapping availability records.
            query = """
                SELECT c.*, MAX(ca.end_time) as end_time
                FROM crew c
        ...
        rules_res = check_rules_from_data(crew_data)
        total_available = sum(1 for c in crew_data if c["available"])
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                # Efficiently check rules by fetching all crew with availability status.
                # MAX(ca.id) combined with GROUP BY ensures robustness against
                # duplicate records while maintaining performance.
                query = """
                    SELECT role, skills, (MAX(ca.id) IS NOT NULL) as available
                    FROM crew c
                    LEFT JOIN crew_availability ca ON c.id = ca.crew_id
                        AND ca.start_time <= ? AND ca.end_time > ?
                    GROUP BY c.id
                """
                crew_rows = conn.execute(query, (now, now)).fetchall()
                crew_data = [dict(r) for r in crew_rows]
                return jsonify(
                    base["available"] and check_rules_from_data(crew_data)["rules_pass"]
                )
    ...
    with get_db() as conn:
        ...
        if name == "P22P6":
            ...
            return {
                ...
                and check_rules_from_data(crew_data)["rules_pass"]
            }
