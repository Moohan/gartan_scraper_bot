...
def get_db():
    """
    ⚡ Optimization: Connection pooling/persistence using Flask's 'g' object.
    Reuse the same connection for multiple queries during a single request.
    """
    try:
        if "db" not in g:
        ...
        # Fallback for scripts running outside of a Flask request context
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """
    Check business rules for the available crew members.
    ⚡ Optimization: Accepts pre-fetched crew dictionaries instead of IDs
...
def root():
    try:
        ...
        # This replaces calling get_availability for every crew member.
        query = """
...
def app_avail(name):
    try:
        ...
        if name == "P22P6":
            # Fallback for individual appliance endpoint to keep performance logic consistent
            ...
            avail_crew = [dict(r) for r in avail_rows]
            return jsonify(base["available"] and check_rules(avail_crew)["rules_pass"])
