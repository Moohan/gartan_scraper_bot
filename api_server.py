...
def format_availability(end_time: Optional[datetime], now: datetime) -> Dict:
    """Helper to format availability status and duration display."""
    if not end_time:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        ...
        return format_availability(curr["end_time"] if curr else None, now)
...
def check_rules(crew_rows: List[Dict]) -> Dict:
    """Evaluate business rules for P22P6 availability based on crew skills."""
    if not crew_rows:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        now = datetime.now()
        conn = get_db()
        # ⚡ Optimization: Use a single JOIN to fetch all crew and their current availability
        # instead of N+1 queries. Connection persistence via flask.g also reduces overhead.
        rows = conn.execute(
            """
        ...
        for r in rows:
            c_dict = dict(r)
            end_time = c_dict.pop("end_time")
            avail = format_availability(end_time, now)
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                ...
                crew_data = [dict(r) for r in rows]
                return jsonify(
