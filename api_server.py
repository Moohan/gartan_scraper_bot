...
def format_availability_data(data: Any, now: datetime) -> Dict:
    """Standardize raw DB availability data into a consistent UI dictionary."""
    try:
        # data can be a sqlite3.Row or a dict
...
def get_currently_available_crew(conn: sqlite3.Connection, now: datetime) -> List[Dict]:
    """Fetch all crew members currently available in a single JOIN query."""
    # MAX(ca.end_time) ensures we get the latest end time if multiple overlap
    query = """
        ...
        GROUP BY c.id
    """
    return [dict(r) for r in conn.execute(query, (now, now)).fetchall()]
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Evaluate business rules based on currently available crew members."""
...
def root():
    try:
        ...
        # ⚡ Optimization: Fetch all crew and their current availability in a single JOIN
