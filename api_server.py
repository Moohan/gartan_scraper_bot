...
def get_db():
    try:
        ...
        # Fallback for when we are outside of application context (e.g. tests or scripts)
...
def format_availability_data(row: sqlite3.Row, now: datetime) -> Dict:
    """Standardizes availability formatting from a DB row containing end_time."""
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    # Whitelist table names to prevent SQL injection as they are dynamic
...
def check_rules(available_crew: List[Dict]) -> Dict:
    """Checks station rules based on a list of available crew member dictionaries."""
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # ⚡ Bolt: Use JOIN to fetch crew and current availability in one batch (prevents N+1)
            rows = conn.execute(
                """
        ...
        crew_data = []
        for r in rows:
            avail_info = format_availability_data(r, now)
            crew_data.append({**dict(r), **avail_info})
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                # ⚡ Bolt: Fetch role and skills in the same query as availability to avoid N+1
