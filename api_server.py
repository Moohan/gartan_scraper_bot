...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules against a list of crew data (roles and skills)."""
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Optimized: Single JOIN query to fetch crew and current availability
            # We use MAX(end_time) and GROUP BY to handle potential overlapping blocks
            query = """
            ...
            # Use refined business logic helper to avoid extra DB calls
