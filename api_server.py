...
def check_rules(
    available_ids: List[int], crew_rows: Optional[List[Dict]] = None
) -> Dict:
...
def root():
    try:
        now = datetime.now()
        conn = get_db()
        # Bolt ⚡: Optimized with a single LEFT JOIN to fetch crew and current availability
        # This eliminates the N+1 query problem where we were querying availability
        # for every single crew member individually.
        query = """
            SELECT c.*, ca.end_time as avail_end
            FROM crew c
        ...
        crew_rows = [dict(r) for r in rows]

        crew_data = []
        for r in crew_rows:
            avail_end = r.pop("avail_end", None)
            if avail_end:
                end_time = parse_dt(avail_end)
                duration_min = int((end_time - now).total_seconds() / 60)
                display = end_time.strftime("%H:%M")
                if end_time.date() == now.date():
                    display += " today"
        ...
        rules_res = check_rules(avail_ids, crew_rows=crew_data)
