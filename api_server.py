...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # ⚡ Optimization: Use a single JOIN query to fetch all crew members and their current availability.
            # This solves the N+1 query problem by replacing N individual availability checks with one batch query.
            # A subquery is used in the LEFT JOIN to ensure each crew member is counted at most once,
            # even if multiple overlapping availability blocks exist (safety requirement).
            rows = conn.execute(
                """
                SELECT c.*, ca.max_end_time as current_end_time
                FROM crew c
                LEFT JOIN (
            ...
            for r in rows:
                c = dict(r)
                end_time = parse_dt(c.pop("current_end_time"))
                if end_time:
                    duration_min = int((end_time - now).total_seconds() / 60)
                    c.update(
                        {
