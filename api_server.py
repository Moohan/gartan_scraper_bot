...
def format_availability_data(end_time: datetime, now: datetime) -> Dict:
...
def _get_available_crew_details(conn, now: datetime) -> List[Dict]:
    """Helper to fetch details of currently available crew members for rule checking."""
    query = """
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        ...
        return format_availability_data(parse_dt(curr["end_time"]), now)
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # ⚡ Optimized: Single query to fetch all crew and their current availability (resolves N+1)
            query = """
