...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    # Explicitly whitelist tables to prevent SQL injection
    if table == "crew_availability":
        ...
        raise ValueError(f"Invalid availability table: {table}")
...
def check_rules(available_ids: List[int]) -> Dict:
    ...
    # Avoid dynamic SQL IN clause to satisfy security scanners.
    # Fetch all crew and filter in Python for this small station dataset.
    with get_db() as conn:
