...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    # Use explicit queries for whitelisted tables to avoid SQL injection risks and satisfy static analysis
...
def check_rules(available_ids: List[int]) -> Dict:
    ...
    rows = []
    with get_db() as conn:
        # Fetch each crew member individually to avoid dynamic SQL construction for IN clauses,
        # which satisfies strict static analysis tools (Sourcery/Bandit).
        for cid in available_ids:
            row = conn.execute(
                "SELECT role, skills FROM crew WHERE id = ?", (cid,)
            ).fetchone()
            if row:
                rows.append(row)
...
if __name__ == "__main__":
    # Binding to 0.0.0.0 is required for containerized environments to allow external access.
