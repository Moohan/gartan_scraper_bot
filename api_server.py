...
def check_rules_from_data(crew_rows: List[Dict]) -> Dict:
    if not crew_rows:
        return {
            "rules_pass": False,
            "rules": {
...
def root():
    try:
        ...
        # Batch fetch crew and their current availability using a JOIN
        # Use MAX(ca.end_time) and GROUP BY to handle potential duplicate availability records safely
        query = """
        ...
        for r in rows:
            c = dict(r)
            end_time_raw = c.pop("end_time", None)
            if end_time_raw:
        ...
        # Get appliance P22P6 availability
