...
def format_hours(minutes: Optional[int]) -> Optional[str]:
    if minutes is None or minutes <= 0: return None
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    with get_db() as conn:
        curr = conn.execute(f"SELECT end_time FROM {table} WHERE {col} = ? AND start_time <= ? AND end_time > ? LIMIT 1", (entity_id, now, now)).fetchone()
        if not curr: return {"available": False, "duration": None, "end_time_display": None}

        end_time = parse_dt(curr["end_time"])
        duration_min = int((end_time - now).total_seconds() / 60)

        display = end_time.strftime("%H:%M")
        if end_time.date() == now.date(): display += " today"
        elif end_time.date() == (now + timedelta(days=1)).date(): display += " tomorrow"
        else: display += end_time.strftime(" on %d/%m")

        return {"available": True, "duration": format_hours(duration_min), "end_time_display": display}

def get_weekly_stats(crew_id: int) -> Dict:
    start, end = get_week_boundaries()
    now = datetime.now()
    with get_db() as conn:
        rows = conn.execute("SELECT start_time, end_time FROM crew_availability WHERE crew_id = ? AND end_time > ? AND start_time < ?", (crew_id, start, end)).fetchall()
        planned, achieved = [], []
        for r in rows:
            s, e = parse_dt(r[0]), parse_dt(r[1])
            planned.append((max(s, start), min(e, end)))
            if e <= now: achieved.append((max(s, start), e))
            elif s < now: achieved.append((max(s, start), now))

        def total_hrs(p): return sum((e - s).total_seconds() / 3600 for s, e in merge_periods(p))
        crew = conn.execute("SELECT contract_hours FROM crew WHERE id = ?", (crew_id,)).fetchone()
        if not crew: return {"error": "Not found"}

        achieved_val = total_hrs(achieved)
        contract_str = crew["contract_hours"] or "0"
        contract = float(contract_str.split()[0]) if contract_str and contract_str[0].isdigit() else 0
        return {
...
def check_rules(available_ids: List[int]) -> Dict:
    if not available_ids: return {"rules_pass": False, "rules": {}, "skill_counts": {"TTR": 0, "LGV": 0, "BA": 0}, "ba_non_ttr": 0}
    with get_db() as conn:
        placeholders = ",".join("?" * len(available_ids))
        rows = conn.execute(f"SELECT role, skills FROM crew WHERE id IN ({placeholders})", available_ids).fetchall()
    ...
    for r in rows:
        ...
        for s in c_skills:
            if s in skills: skills[s] += 1
        if "BA" in c_skills and "TTR" not in c_skills: ba_non_ttr += 1
        if r["role"] in ["FFC", "CC", "WC", "CM"] and "BA" in c_skills: ffc_ba = True

    rules = {"total_crew_ok": len(rows) >= 4, "ttr_present": skills["TTR"] > 0, "lgv_present": skills["LGV"] > 0, "ba_non_ttr_ok": ba_non_ttr >= 2, "ffc_with_ba": ffc_ba}
    return {"rules_pass": all(rules.values()), "rules": rules, "skill_counts": skills, "ba_non_ttr": ba_non_ttr}
...
def root():
    try:
        ...
        crew_data.sort(key=lambda x: (not x["available"], ranks.get(x["role"], 99), x["name"]))
        ...
        with get_db() as conn:
            app_p22 = conn.execute("SELECT id FROM appliance WHERE name = 'P22P6'").fetchone()
            if app_p22: p22p6_base = get_availability(app_p22["id"], "appliance_availability", now)

        p22p6_avail = p22p6_base["available"] and rules_res["rules_pass"]

        return render_template_string(DASHBOARD_TEMPLATE,
            crew_data=crew_data, now=now, total_available=len(avail_ids),
            p22p6_avail=p22p6_avail, p22p6_duration=p22p6_base["duration"] if p22p6_avail else None,
            rules=rules_res["rules"], skill_counts=rules_res["skill_counts"])
    except Exception as e:
        logger.error(f"Root error: {e}")
        return jsonify({"error": "Internal Server Error", "service": "Gartan Scraper Bot", "Dashboard": "Unavailable"}), 500

@app.route("/crew")
def list_crew():
    try: return jsonify(get_crew_list())
...
def crew_avail(id):
    try:
        ...
        with get_db() as conn:
            if not conn.execute("SELECT 1 FROM crew WHERE id = ?", (id,)).fetchone(): return jsonify({"error": "Not found"}), 404
...
def crew_hrs_planned(id):
    try:
        res = get_weekly_stats(id)
        return jsonify({"hours_planned_week": res["hours_planned_week"]}) if "error" not in res else (jsonify(res), 404)
    except: return jsonify({"error": "DB error"}), 500
...
def crew_contract(id):
    try:
        res = get_weekly_stats(id)
        return jsonify({"contract_hours": res["contract_hours"]}) if "error" not in res else (jsonify(res), 404)
    except: return jsonify({"error": "DB error"}), 500
...
def crew_hrs_achieved(id):
    try:
        res = get_weekly_stats(id)
        return jsonify({"hours_achieved": res["hours_achieved"]}) if "error" not in res else (jsonify(res), 404)
    except: return jsonify({"error": "DB error"}), 500
...
def crew_hrs_rem(id):
    try:
        res = get_weekly_stats(id)
        return jsonify({"hours_remaining": res["hours_remaining"]}) if "error" not in res else (jsonify(res), 404)
    except: return jsonify({"error": "DB error"}), 500
...
def app_dur(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                avail_ids = [r[0] for r in conn.execute("SELECT crew_id FROM crew_availability WHERE start_time <= ? AND end_time > ?", (now, now)).fetchall()]
                if not (base["available"] and check_rules(avail_ids)["rules_pass"]): return jsonify(None)
