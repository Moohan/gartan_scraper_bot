...
def format_availability_display(end_time: datetime, now: datetime) -> str:
    """Format availability end time for display."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Wrapper for check_rules_from_data for backwards compatibility."""
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules from a list of crew data dicts."""
    if not crew_data:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        ...
        # ⚡ Performance: Fetch all crew members and their current availability in a single JOIN query.
        # This replaces the previous N+1 query pattern and significantly reduces database roundtrips.
        query = """
