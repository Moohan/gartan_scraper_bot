...
def format_availability_display(end_time: datetime, now: datetime) -> str:
    """Standardize the display of availability 'Until' time."""
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Evaluate business rules based on provided crew data."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Check rules for a list of crew IDs by filtering all crew data in memory."""
    ...
    # Fetch all crew (cached/reused connection) and filter in memory to avoid dynamic SQL
    all_crew = get_crew_list()
    avail_crew = [c for c in all_crew if c["id"] in available_ids]
    return check_rules_from_data(avail_crew)
...
def root():
    try:
        ...
        # ⚡ Performance: Use a single JOIN to fetch all crew members and their current availability in one go.
        # This resolves the N+1 query problem in the dashboard route.
        query = """
        ...
        for r in rows:
            data = dict(r)
            available = data["end_time"] is not None
            if available:
        ...
        # ⚡ Performance: Use already fetched data for business rules check instead of re-querying.
