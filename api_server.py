...
def format_availability_data(row: Any, now: datetime) -> Dict:
    """Format availability from a database row or dictionary."""
    try:
...
def check_rules(
    available_ids: Optional[List[int]], crew_rows: Optional[List[Any]] = None
) -> Dict:
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Optimized batch fetch for all crew and their current availability
            rows = conn.execute(
                """
            ...
            for r in rows:
                ...
                c_dict = dict(r)
                c_dict.pop("end_time", None)
                crew_data.append({**c_dict, **avail})
                if avail["available"]:
                    avail_ids.append(r["id"])
                    available_crew_rows.append(r)
        ...
        rules_res = check_rules(avail_ids, crew_rows=available_crew_rows)
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                ...
                return jsonify(
                    base["available"]
                    and check_rules(None, crew_rows=crew_rows)["rules_pass"]
                )
