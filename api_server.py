...
def _calculate_rules_logic(crew_rows: List[Dict]) -> Dict:
    """Core logic for evaluating station business rules based on available crew."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Check station business rules for a list of crew IDs."""
    ...
    with get_db() as conn:
        ...
        return _calculate_rules_logic([dict(r) for r in rows])
...
def root():
    try:
        ...
        # ⚡ Optimization: Fetch all availabilities in one query instead of N
        availabilities = get_all_availability("crew_availability", now)
        ...
        # ⚡ Optimization: Use already-fetched crew data for business rules
        rules_res = _calculate_rules_logic(available_crew)

        # ⚡ Optimization: Appliance availability can also be fetched more efficiently
        appliance_avails = get_all_availability("appliance_availability", now)
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                # ⚡ Optimization: Fetch roles/skills in a single JOIN query instead of two steps
                rows = conn.execute(
                    """
...
def get_appliance_available_data(name):
    ...
    with get_db() as conn:
        ...
        base = get_availability(app["id"], "appliance_availability", now)
        if name == "P22P6":
            # ⚡ Optimization: Fetch roles/skills in a single JOIN query
            rows = conn.execute(
                """
                SELECT c.role, c.skills
                FROM crew c
