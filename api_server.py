...
def get_weekly_stats(crew_id: int) -> Dict:
    ...
    contract_str = crew_row["contract_hours"] or "0"
    contract = (
...
def check_rules(available_ids: List[int], crew_rows: Optional[List] = None) -> Dict:
    if not available_ids and not crew_rows:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        ...
        # Optimized: Fetch all crew and their current availability in one query using JOIN
        query = """
        ...
        avail_ids = [c["id"] for c in avail_crew]

        # Optimization: Pass the already fetched crew details to check_rules
        rules_res = check_rules(avail_ids, crew_rows=avail_crew)
