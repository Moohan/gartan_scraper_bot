...
def format_availability_data(end_time_val: Any, now: datetime) -> Dict:
    """Standardize the conversion of database availability into the display dictionary."""
    if not end_time_val:
...
def get_currently_available_crew(conn, now: datetime) -> List[Dict]:
    """Fetch all crew members currently available with their metadata in a single JOIN."""
    rows = conn.execute(
        """
...
def check_rules(crew_members: List[Dict]) -> Dict:
    """Evaluate business rules against a list of pre-fetched crew dictionaries."""
    if not crew_members:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Batch fetch all crew and their current availability in one JOIN
            rows = conn.execute(
                """
                SELECT c.*, ca.end_time
                FROM crew c
            ...
            for r in rows:
                c = dict(r)
                avail_info = format_availability_data(c.get("end_time"), now)
                c.update(avail_info)
                crew_data.append(c)
                if c["available"]:
                    avail_crew.append(c)
            ...
            # Fetch P22P6 availability in a single JOIN
            p22p6_base = {"available": False, "duration": None}
            app_row = conn.execute(
                """
                SELECT a.id, aa.end_time
                FROM appliance a
                LEFT JOIN appliance_availability aa ON a.id = aa.appliance_id AND aa.start_time <= ? AND aa.end_time > ?
                WHERE a.name = 'P22P6'
                LIMIT 1
                """,
                (now, now),
            ...
            if app_row:
                p22p6_base = format_availability_data(app_row[0]["end_time"], now)
...
def app_avail(name):
    try:
        ...
        with get_db() as conn:
            ...
            if name == "P22P6":
                avail_crew = get_currently_available_crew(conn, now)
                return jsonify(
