...
def format_availability_data(row, now: datetime) -> Dict:
    """Format raw availability row into a display-friendly dictionary."""
    if not row or "end_time" not in row.keys() or not row["end_time"]:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    """Get current availability for a specific entity."""
    col = "crew_id" if table == "crew_availability" else "appliance_id"
    with get_db() as conn:
        sql = (
            f"SELECT end_time FROM {table} "
            f"WHERE {col} = ? AND start_time <= ? AND end_time > ? LIMIT 1"
        )
...
def root():
    try:
        now = datetime.now()
        with get_db() as conn:
            # Single query with JOIN to get all crew and their current availability
            rows = conn.execute(
                """
