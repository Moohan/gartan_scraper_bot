...
def close_db(error):
    """Ensure the database connection is closed at the end of the request."""
    db = g.pop("db", None)
    if db is not None:
        db.close()
...
def format_availability_display(now: datetime, end_time: datetime) -> str:
...
def get_availability(entity_id: int, table: str, now: datetime) -> Dict:
    ...
    return {
        ...
        "end_time_display": format_availability_display(now, end_time),
    }
...
def check_rules_from_data(crew_data: List[Dict]) -> Dict:
    """Check business rules based on provided crew data (requires 'role' and 'skills')."""
...
def check_rules(available_ids: List[int]) -> Dict:
    """Backward compatibility wrapper for checking rules by IDs."""
...
def root():
    try:
        ...
        # ⚡ Optimization: Single query with LEFT JOIN to fetch all crew and current availability.
        # This replaces the N+1 connection pattern that previously triggered for every crew member.
        query = """
        ...
        for r in rows:
            c = dict(r)
            end_time = c.pop("current_end_time")
            if end_time:
                end_dt = parse_dt(end_time)
                duration_min = int((end_dt - now).total_seconds() / 60)
                c.update(
                    {
                        "available": True,
                        "duration": format_hours(duration_min),
                        "end_time_display": format_availability_display(now, end_dt),
                    }
                )
            else:
                c.update(
        ...
        # Check P22P6 appliance availability
