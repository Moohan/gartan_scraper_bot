...
def check_rules_from_data(crew_list: List[Dict]) -> Dict:
    if not crew_list:
        return {
            "rules_pass": False,
            "rules": {},
...
def root():
    try:
        ...
        # Batch query for all crew and their current availability
        # We use a subquery to find the current availability block if it exists
        query = """
        ...
        for r in rows:
            c = dict(r)
            available = c["end_time"] is not None
            display_data = {
                "available": available,
                "duration": None,
            ...
            if available:
                ...
                duration_min = int((end_time - now).total_seconds() / 60)
                display_data["duration"] = format_hours(duration_min)
                display_data["end_time_display"] = format_availability_display(
                    end_time, now
                )

            crew_data.append({**c, **display_data})
