...
def test_crew_endpoints() -> bool:
    ...
    try:
        ...
        response = requests.get(
            f"{API_BASE}/v1/crew/{crew_id}/available", timeout=TIMEOUT
        )
        if response.status_code != 200:
        ...
        # Test duration
        response = requests.get(
            f"{API_BASE}/v1/crew/{crew_id}/duration", timeout=TIMEOUT
        )
        if response.status_code != 200:
...
def test_appliance_endpoints() -> bool:
    ...
    try:
        ...
        # Test availability
        response = requests.get(
            f"{API_BASE}/v1/appliances/{appliance_name}/available", timeout=TIMEOUT
        )
        if response.status_code != 200:
            print(
                f"❌ Appliance availability failed with status {response.status_code}"
            )
            return False
        ...
        # Test duration
        response = requests.get(
            f"{API_BASE}/v1/appliances/{appliance_name}/duration", timeout=TIMEOUT
        )
        if response.status_code != 200:
...
def test_error_handling() -> bool:
    ...
    try:
        ...
        response = requests.get(
            f"{API_BASE}/v1/appliances/INVALID/available", timeout=TIMEOUT
        )
        if response.status_code != 404:
