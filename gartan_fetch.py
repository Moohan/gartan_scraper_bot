...
def gartan_login_and_get_session():
    ...
    print(
        f"[DEBUG] gartan_login called: username={'[REDACTED]' if username else 'None'}, password_set={'yes' if password else 'no'}, cached_session={'yes' if _authenticated_session is not None else 'no'}"
    )
...
def _post_schedule_request(session, schedule_url, payload, headers, booking_date):
    ...
    """
    if not session:
        log_debug("warn", f"No session available for schedule fetch of {booking_date}")
        return None
