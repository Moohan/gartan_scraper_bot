...
def gartan_login_and_get_session():
    ...
    # Temporary debug: log cached session presence for bisecting test-order flakiness (username/password redacted for security)
    print(
        f"[DEBUG] gartan_login called: cached_session={'yes' if _authenticated_session is not None else 'no'}"
    )
