...
def fetch_grid_html_for_date(session, booking_date):
    ...
    """
    if session is None:
        log_debug("warn", f"No session for fetching grid on {booking_date}")
        return None
...
def _post_schedule_request(session, schedule_url, payload, headers, booking_date):
    ...
    if session is None:
        log_debug(
            "warn",
            f"No session available for schedule request on {booking_date} (running in cache-only or no-auth mode)",
        )
        return None
    import json
