...
## 2025-02-10 - JOIN vs N+1 in SQLite Dashboard
**Learning:** Batch fetching crew availability using a `LEFT JOIN` and `MAX(end_time)` with `GROUP BY c.id` reduced dashboard latency by ~48%. Refactoring `check_rules` to use pre-fetched metadata further eliminated redundant DB roundtrips.
**Action:** Always look for `for` loops that call `get_availability` or similar DB-querying functions. Replace them with single queries using JOINs and move formatting logic into reusable helpers.
