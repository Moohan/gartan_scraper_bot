...
## 2026-02-07 - SQLite N+1 and Connection Overhead

**Learning:** Opening a new SQLite connection for every small query in a loop (N+1 problem) is extremely expensive. In this codebase, 50 crew members resulted in 50+ connections per dashboard request.

**Action:** Implement per-request connection caching using Flask's `g` object and use SQL `JOIN`s to batch data retrieval. This resulted in a 2.3x speedup (25ms to 11ms) for the dashboard endpoint.
