...
## 2025-05-14 - Dashboard N+1 Query Optimization
**Learning:** The dashboard route had a significant N+1 query bottleneck, fetching crew availability one by one. Using a single LEFT JOIN query with start/end time filters reduced response latency by ~50% for 50 crew members.
**Action:** Always check for loops that perform database queries and replace them with batch lookups or JOINs.
