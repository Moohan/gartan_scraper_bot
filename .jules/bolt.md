...
## 2026-01-30 - Dashboard N+1 Scalability Bottleneck

**Learning:** The root dashboard route had an N+1 query pattern where it fetched all crew members and then performed individual availability and rule-check lookups for each. While fast for small crew lists, this pattern scales poorly and puts unnecessary load on the database.

**Action:** Use batch fetching helpers like `get_all_availability` and separate business logic from data retrieval to allow processing already-fetched data in memory.
