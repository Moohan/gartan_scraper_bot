...
new_lines.append("    \"\"\"Get start date and effective max days, aligned to current week start.\"\"\"\n")
new_lines.append("    now = get_now()\n")
new_lines.append("    # Start from Monday of current week\n")
new_lines.append("    start_date = (now - timedelta(days=now.weekday())).replace(hour=0, minute=0, second=0, microsecond=0)\n")
