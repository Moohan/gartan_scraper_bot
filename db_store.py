...
def defrag_availability(db_conn=None):
    ...
    try:
        for table_name in ["crew_availability", "appliance_availability"]:
            # Simple iterative merging logic:
            # 1. Select all blocks sorted by id and start_time
            if table_name == "crew_availability":
                c.execute(
            ...
            for i in range(1, len(rows)):
                ...
                if curr_id == prev_id and curr_start <= prev_end:
                    ...
                    if table_name == "crew_availability":
                        c.execute(
                            "DELETE FROM crew_availability WHERE id = ?",
                            (curr_row_id,),
                        )
                    else:
                        c.execute(
            ...
            if merged_count > 0:
                logger.info(f"Merged {merged_count} blocks in {table_name}")
