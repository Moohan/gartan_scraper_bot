...
def defrag_availability(db_conn=None):
    ...
    try:
        for table in ["crew_availability", "appliance_availability"]:
            # Use hardcoded queries to avoid dynamic SQL construction (Bandit B608)
            if table == "crew_availability":
                select_q = "SELECT id, crew_id, start_time, end_time FROM crew_availability ORDER BY crew_id, start_time"
                update_q = "UPDATE crew_availability SET end_time = ? WHERE id = ?"
                delete_q = "DELETE FROM crew_availability WHERE id = ?"
            else:  # appliance_availability
                select_q = "SELECT id, appliance_id, start_time, end_time FROM appliance_availability ORDER BY appliance_id, start_time"
                update_q = "UPDATE appliance_availability SET end_time = ? WHERE id = ?"
                delete_q = "DELETE FROM appliance_availability WHERE id = ?"
            ...
            for i in range(1, len(rows)):
                ...
                if curr_id == prev_id and curr_start <= prev_end:
                    ...
                    if new_end != prev_end:
                        # Update prev block
                        c.execute(update_q, (new_end, prev_row_id))
                        prev_end = new_end

                    # Delete current block
                    c.execute(delete_q, (curr_row_id,))
                    merged_count += 1
                else:
                    # Move to next block
                    prev_id, _, prev_end, prev_row_id = (
