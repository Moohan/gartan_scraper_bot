...
def check():
    ...
    rows = conn.execute(f"SELECT * FROM crew WHERE name IN ({placeholders})", crew_names).fetchall()

    for r in rows:
        print(f"ID: {r['id']}, Name: {r['name']}, Role: {r['role']}, Skills: {r['skills']}, Contact: {r['contact']}")
