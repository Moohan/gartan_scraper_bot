...
def print_duration_questions(data):
    ...
    if available_crew:
        print("  (For currently available crew - how long until they become unavailable?)")
...
def print_business_rules_questions(data):
    ...
    ba_count = sum(1 for _, skills, _ in available_with_skills
                   if 'BA' in skills and 'TTR' not in skills)
    ffc_plus_ba = any('BA' in skills and 'TTR' not in skills and role in ['WC', 'CC', 'FFC']
                      for _, skills, role in available_with_skills)
    
    should_be_available = (total_available >= 4 and ttr_available and lgv_available and
                           ba_count >= 2 and ffc_plus_ba)
    
    # Format status
    def format_status(condition):
        return '✅ YES' if condition else '❌ NO'
    
    print(f"  Total Crew ≥4      | API: {format_status(total_available >= 4):<6} | Real: YES/NO ____ (Count: ____)")
    print(f"  TTR Skill Present  | API: {format_status(ttr_available):<6} | Real: YES/NO ____")
    print(f"  LGV Skill Present  | API: {format_status(lgv_available):<6} | Real: YES/NO ____")
    print(f"  ≥2 BA (non-TTR)    | API: {format_status(ba_count >= 2):<6} | Real: YES/NO ____ (Count: ____)")
    print(f"  ≥1 FFC+ with BA    | API: {format_status(ffc_plus_ba):<6} | Real: YES/NO ____")
    
    print("\n🚨 OVERALL P22P6 DECISION")
    print("-" * 40)
    api_appliance = data['appliance_status']
    print(f"  Should Operate     | API: {format_status(should_be_available):<6} | Real: YES/NO ____")
    print(f"  Actually Available | API: {format_status(api_appliance):<6} | Real: YES/NO ____")
